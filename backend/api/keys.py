...
def _out(row) -> dict:
    """Row → serializable dict with scopes expanded to a sorted list."""
    return {**row.__dict__, "scopes": sorted(api_key_service.scopes_of(row))}
...
def list_keys(user: User = Depends(get_current_user),
    ...
    return [ApiKeyOut.model_validate(_out(k)) for k in api_key_service.list(db, user)]
...
def create_key(payload: ApiKeyCreate, user: User = Depends(get_current_user),
    ...
    row, raw = api_key_service.create(db, user, payload.name, payload.scopes)
    return ApiKeyCreated.model_validate({**_out(row), "key": raw})
