...
/** Public, crawlable Developers page — API reference + pricing. Dark, animated
 *  chrome shared with the landing page. */
...
function SectionTitle({ icon: Icon, children }: { icon: typeof Code2; children: ReactNode }) {
  return (
    <h2 className="flex items-center gap-2 text-2xl font-bold tracking-tight text-white">
      <Icon className="h-5 w-5 text-blue-400" /> {children}
    </h2>
  );
}
  ...
  return (
    <div className="min-h-screen bg-[#060a13] text-white">
      ...
      <main className="mx-auto w-full max-w-6xl px-6">
        {/* Hero */}
        <section className="relative isolate max-w-2xl py-16 md:py-20">
          ...
          <h1 className="mt-5 text-4xl font-bold tracking-tight text-white sm:text-5xl">The Finverse API</h1>
          ...
          <div className="mt-7 flex flex-wrap gap-3">
            <Link to="/settings" className="inline-flex items-center gap-2 rounded-lg bg-white px-5 py-3 text-sm font-semibold text-zinc-900 transition-colors hover:bg-zinc-200">
              Get an API key <ArrowRight className="h-4 w-4" />
            </Link>
            <a href={DOCS_URL} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-lg border border-white/15 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-white/5">
        ...
        {/* Quick start */}
        <section className="space-y-5 border-t border-white/10 py-16">
          ...
          <div className="flex flex-wrap items-center gap-2 rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm">
            ...
            <span className="font-mono text-zinc-200">{API_BASE}</span>
          </div>
        ...
        {/* Endpoints */}
        <section className="space-y-6 border-t border-white/10 py-16">
          ...
          <p className="max-w-2xl text-zinc-400">
            ...
            <a href={DOCS_URL} target="_blank" rel="noreferrer" className="font-medium text-blue-400 hover:underline">
                  ...
                  {g.endpoints.map((e) => (
                    <li key={e.path} className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <code className="text-[13px] text-zinc-200">{e.path}</code>
                      </div>
        ...
        {/* Pricing */}
        <section id="pricing" className="scroll-mt-20 space-y-8 border-t border-white/10 py-16">
          <div className="text-center">
            <h2 className="text-3xl font-bold tracking-tight text-white">Simple, usage-based pricing</h2>
            <p className="mt-3 text-zinc-400">Start free. Upgrade when you ship. Cancel anytime.</p>
          </div>
            ...
            {PLANS.map((p) => (
              <div
                ...
                className={cn(
                  ...
                  p.highlight ? "border-blue-500/50 bg-blue-500/[0.04] shadow-xl shadow-blue-500/10" : "border-white/10",
                )}
              >
                {p.highlight && (
                  <span className="absolute -top-3 left-7 rounded-full bg-blue-600 px-3 py-0.5 text-[11px] font-semibold text-white">
                    Most popular
                  </span>
                )}
                ...
                <Link
                  ...
                  className={cn(
                    ...
                    p.highlight ? "bg-white text-zinc-900 hover:bg-zinc-200" : "border border-white/15 text-white hover:bg-white/5",
                  )}
                >
                  {p.cta.label}
                </Link>
        ...
        {/* FAQ */}
        <section className="space-y-6 border-t border-white/10 py-16">
          <h2 className="text-3xl font-bold tracking-tight text-white">FAQ</h2>
          <div className="grid gap-5 md:grid-cols-2">
            {[
